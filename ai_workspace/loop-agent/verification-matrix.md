...
`scripts/ci-tests.sh` 会读取 `package.json` 并依次运行已存在的 `typecheck`、`test` 和 `build` 脚本。当前行为测试使用 Node.js 内置 test runner 检查首页、弹窗与任务看板，以及后端 API 与前端后端化契约；真实浏览器点击可作为按需补充证据。带后端接口的后续需求须先通过 Product Requirement、Dependency Analysis 和 API Documentation 校验器，再生成 DAG。
