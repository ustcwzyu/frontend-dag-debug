...
使用能够证明声明的最窄命令。该项目是 Vite + TypeScript 前端调试项目；治理检查由 loop-agent 生成，项目验证以 TypeScript 类型检查、Node.js 行为测试和 Vite 生产构建为准。

| 声明 | 最低验证 | 更强验证 |
|---|---|---|
| loop-agent 治理有效 | `bash scripts/check-repo.sh` | `bash scripts/ci-governance.sh` |
| TypeScript 类型有效 | `npm run typecheck` | `bash scripts/ci-tests.sh` |
...
`scripts/ci-tests.sh` 会读取 `package.json` 并依次运行已存在的 `typecheck`、`test` 和 `build` 脚本。当前行为测试使用 Node.js 内置 test runner 检查首页、弹窗与任务看板；真实浏览器点击可作为按需补充证据。带后端接口的后续需求须先通过 Product Requirement、Dependency Analysis 和 API Documentation 校验器，再生成 DAG。
