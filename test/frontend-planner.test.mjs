...
test('[AC-PLN-002] 静态契约：main 渲染标题、本机说明、挂载点与工作台接线', () => {
  ...
  assert.match(mainSource, /initPlannerWorkbench\(host,\s*\{/)
  assert.match(mainSource, /hasJournalDraft:\s*\(\) => hasJournalDraftContent\(loadJournalDraft\(\)\)/)
  assert.match(mainSource, /startJournalSession:\s*\(snapshot\) => startJournalSession\(snapshot\)/)
  assert.match(mainSource, /navigateToProgress:\s*\(\) => navigate\('#\/progress'\)/)
