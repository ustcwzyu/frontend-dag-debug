...
- `npm test`：`node --test` 运行 `test/homepage.test.mjs`（站点身份、默认唯一选中、hash 路由导航、轨迹四阶段、六类能力、本周实验、第一课深化：结构/对照表/八步闭环/静态 run 样例/五阶段实验/五个模板/评估量表/自测题/静态回归断言、无网络/无存储断言、reduced-motion 媒体查询、meta/favicon/README 断言）、`test/task-board.test.mjs`（task-board 纯函数断言全部保留 + 旧首页挂载强绑定断言）、`test/server-api.test.mjs`（后端 API 端到端：内容端点、错误结构、注册/登录/令牌、按用户隔离的进度，内存 SQLite + 临时端口）、`test/frontend-api.test.mjs`（前端契约静态断言：API 客户端端点与形状校验、令牌 key 与 task-board 隔离、降级横幅、进度面板、课程区纯净与 @keyframes 唯一性）、`test/frontend-journal.test.mjs`（学习会话工作台契约：运行时纯函数断言 isTemplateCompleted / computeFirstLessonCompleted / mergeProgressForSync / validateScore / formatDuration / syncJournalProgress 失败语义，静态断言挂载点位于 progress-panel 之后、登录态联动、存储 key 隔离与样式）与 `test/frontend-router.test.mjs`（hash 路由契约：parseHash 固定路由表单元断言、router 零依赖纯净、startRouter 唯一 hashchange 守卫、主导航 is-active、登录门/guest 提示、cachedContent 缓存与 contentLoadFailed 降级、getElementById 替换路径、工作台节点复用、404 兜底与课程占位静态断言）。
